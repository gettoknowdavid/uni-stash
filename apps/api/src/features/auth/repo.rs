...
impl AuthRepo {
    ...
    pub async fn get_user_stats(&self, user_id: &uuid::Uuid) -> Result<(i64, i64, i64), AppError> {
        let row = sqlx::query!(
            "SELECT
               ...
               (SELECT COUNT(*) FROM saved_items WHERE user_id = $1) AS saved",
            user_id,
        )
        .fetch_one(&self.db)
        .await?;
        Ok((
            ...
            row.sold.unwrap_or(0),
            row.saved.unwrap_or(0),
        ))
    }
