...
#[sqlx::test]
async fn non_admin_cannot_create_school_returns_401(pool: PgPool) {
    // A student token (purpose "access") is rejected by AdminSession at the
    // auth layer with 401 — it never reaches the permission check.
    let state = test_state(pool);
    let app = test::init_service(school_app(state)).await;
    ...
    // Send a completely fabricated (invalid) bearer token
    let req = test::TestRequest::post()
        .uri("/api/v1/schools")
...
#[sqlx::test]
async fn non_admin_cannot_update_school_returns_401(pool: PgPool) {
    // A student token is rejected by AdminSession at the auth layer with 401.
    let result =
        sqlx::query("INSERT INTO schools (name, domain) VALUES ('Uni', 'uni.edu') RETURNING id")
            .fetch_one(&pool)
            .await
...
#[sqlx::test]
async fn non_admin_cannot_delete_school_returns_401(pool: PgPool) {
    // A student token is rejected by AdminSession at the auth layer with 401.
    let result =
        sqlx::query("INSERT INTO schools (name, domain) VALUES ('Uni', 'uni.edu') RETURNING id")
            .fetch_one(&pool)
...
async fn delete_school_with_users_returns_400(pool: PgPool) {
    ...
    // Create a user referencing that school (FK dependency)
