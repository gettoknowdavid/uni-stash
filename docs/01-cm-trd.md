...
| Method | Path                       | Purpose              | Notes                                                                                                                                                                                                            |
| ------ | -------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/listings`                | Browse/filter        | Query params: `category`, `q` (full-text search), `min_price`, `max_price`, `status` (defaults to `active` + `reserved` — reserved cards show a RESERVED badge; `sold`/`deleted` only via explicit `status`), `cursor`, `limit`.                                                                             |
