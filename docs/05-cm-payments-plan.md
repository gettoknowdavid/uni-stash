...
### Gateway-Held Escrow Model — Buyer's Choice of In-App Payment or Off-App (In-Person) Payment
...
2. [Regulatory architecture: why the platform must NOT hold funds](#2-regulatory-architecture-why-the-platform-must-not-hold-funds)
3. [Nigerian regulatory & compliance context](#3-nigerian-regulatory--compliance-context)
4. [Database schema additions](#4-database-schema-additions)
5. [Payment gateway abstraction (multi-provider support)](#5-payment-gateway-abstraction-multi-provider-support)
6. [The payment state machine](#6-the-payment-state-machine)
...
UniStash today is a trust-based classifieds board: buyer and seller find each other, agree a price off-platform, meet in person, and the seller taps "mark as sold." There is no payment processing anywhere in the codebase — no `payments` table, no Flutterwave/Paystack/InterSwitch integration, no checkout endpoint.
...
- **Pay in-app** — funds are collected by the payment gateway and held in escrow by the gateway (not the platform). The platform issues settle/refund instructions via API. The seller gets paid once the buyer confirms receipt (or after an automatic release window).
...
- **The platform never holds buyer funds.** Funds are held in escrow by a CBN-licensed payment gateway (Flutterwave). The platform issues settle/refund instructions via API. This eliminates the need for a CBN MMO/PSP licence (₦5B capital requirement) and avoids co-mingling risk.
...
- **Payment gateway logic is abstracted behind a trait.** The system supports multiple payment gateways (Flutterwave, Paystack, InterSwitch) via a `PaymentGateway` trait. Each gateway implements the trait. A `payment_gateway` column on the `payments` table records which provider handled each transaction.

---

## 2. Regulatory architecture: why the platform must NOT hold funds

This is the most important section in this document. It explains the architectural decision that makes the entire payments feature legal under Nigerian law.

### 2.1 The problem with platform-as-custodian

An earlier draft of this plan designed the platform as a custodian: buyer pays → platform holds funds in its bank account → platform disburses to seller on confirmation. This is **non-compliant with CBN regulations**:

- **Unlicensed custody**: Under CBN licensing frameworks, holding or pooling user funds in a corporate account requires a specific licence — MMO (Mobile Money Operator), Switching, or PSP (Premium Service Provider). The minimum capital requirement is **₦5 billion**. A standard e-commerce platform cannot legally do this.
- **Co-mingling risk**: Holding buyer funds in the platform's bank account until the buyer confirms receipt constitutes illegal co-mingling of corporate and user funds. This exposes the platform to severe AML penalties and immediate CBN shutdown.
- **Gateway limitations**: Paystack, Flutterwave, and InterSwitch are PSSPs (Payment Solution Service Providers) that process transactions. They do not provide regulatory cover for a third-party marketplace to pool and hold user funds indefinitely.

### 2.2 The compliant solution: gateway-held escrow

**Flutterwave has a native escrow API.** When a buyer pays on Flutterwave with the `rave_escrow_tx` metadata flag, Flutterwave (a CBN-licensed Switching & Processing PSP) holds the funds in escrow — not the platform. The platform issues settle or refund instructions via API, and Flutterwave moves the money to the seller's bank account or back to the buyer.

```
OLD (non-compliant):
  Buyer → [money] → Platform's bank account → [money] → Seller
                                     ↑
                          Platform holds funds (needs MMO licence)

NEW (CBN-compliant):
  Buyer → [money] → Flutterwave escrow → [money] → Seller
                                     ↑
                          Flutterwave holds funds (has Switching licence)
                          Platform issues settle/refund instructions via API
```

**This means:**
1. **No CBN licence needed**: The platform operates under Flutterwave's licensed principal relationship, just as any merchant does. The platform never touches raw funds.
2. **No co-mingling**: Buyer funds are never in the platform's bank account. Flutterwave's segregated escrow accounts hold them.
3. **No settlements table**: Since money never enters the platform's account, there's nothing to reconcile on our side. The `payments` table tracks the escrow lifecycle (held → settled/refunded), and Flutterwave handles the actual money movement.
4. **CBN audit trail**: The `payments` and `webhook_events` tables serve as the platform's transaction records. Flutterwave maintains its own regulatory records as the licensed escrow holder.

### 2.3 Why Flutterwave over Paystack for escrow

| Feature | Flutterwave | Paystack |
|---|---|---|
| **Native escrow API** | ✅ Yes — `rave_escrow_tx` metadata holds funds; `/transactions/escrow/settle` releases; `/transactions/escrow/refund` refunds | ❌ No native escrow. Only split payments (funds split at settlement, no hold period) |
| **CBN licence** | Switching & Processing | Switching & Processing |
| **Refund API** | ✅ Full + partial refund via API | ✅ Refund via API |
| **Subaccounts** | ✅ Create subaccounts for sellers | ✅ Create subaccounts for sellers |
| **Verve card support** | ✅ | ✅ |

**Paystack is not suitable for escrow** because it doesn't offer a "hold until release" mechanism. Paystack splits funds immediately at settlement. You'd need a commercial agreement with Paystack to delay settlement, which isn't available as a standard API feature.

**Flutterwave is the primary gateway for MVP.** Paystack and InterSwitch remain future options (the trait abstraction supports them), but Flutterwave's native escrow API is the only one that satisfies the "hold until buyer confirms" requirement without a custom commercial arrangement.

---

## 3. Nigerian regulatory & compliance context
...
### 3.1 Regulatory bodies and applicable laws

| Body / Law | Relevance to UniStash |
|---|---|
| **Central Bank of Nigeria (CBN)** | Regulates all payment service providers and payment systems. CBN licensing guidelines, the Payments System Vision 2028, and the 2024 updated framework for digital escrow services. Flutterwave holds the CBN licence; UniStash operates as a merchant on their platform. |
| **Nigerian Financial Intelligence Unit (NFIU)** | Receives Suspicious Transaction Reports (STRs) from all reporting entities. Any platform facilitating fund transfers is a reporting entity under the Money Laundering (Prevention and Prohibition) Act, 2022 (MLPPA). |
...
### 3.2 CBN escrow and fund-holding requirements

The CBN's 2024 updated framework for digital escrow services introduced key requirements:

1. **Licensing**: Platforms must hold a specific CBN licence (PSSP, Switching, or MMO) **or** operate on top of a licensed payment service provider. **UniStash's approach**: operates through Flutterwave, which holds a CBN Switching & Processing licence. The platform issues settle/refund instructions; Flutterwave holds and moves the money. No separate licence needed.
2. **Segregated accounts**: Client funds must be held in accounts separate from operational finances. **UniStash's approach**: Flutterwave holds buyer funds in their segregated escrow accounts. The platform's bank account never touches buyer funds.
3. **Transaction records**: Detailed records of all transactions must be maintained and made available to CBN on request. **UniStash's approach**: the `payments` and `webhook_events` tables serve as this audit trail (see §4). Flutterwave also maintains its own regulatory records.
...
2. **Data minimisation**: Only collect what's necessary. The platform does **not** store raw card numbers, bank account numbers, or CVV — Flutterwave handles PCI-DSS compliance. UniStash stores only gateway-specific identifiers (reference codes, transaction IDs).
...
### 3.6 What's NOT required for MVP (overkill)

| Requirement | Why it's overkill for MVP |
|---|---|
| **Own CBN payment licence (PSSP/Switching/MMO)** | UniStash operates through Flutterwave (licensed Switching & Processing). The platform never touches funds — Flutterwave holds them in escrow and disburses on instruction. Operating as a merchant on a licensed principal is sufficient. |
...
| **BVN/NIN verification for all users** | CBN's tiered KYC allows transactions up to ₦50,000 without BVN. Flutterwave handles buyer-side KYC at checkout. Seller bank details are verified by Flutterwave's resolve-account endpoint. |
...
| **Full PCI-DSS certification** | Flutterwave handles card data. UniStash never touches raw card numbers. Flutterwave's PCI-DSS compliance covers the card flow. |
| **VAT registration with FIRS** | Required once revenue exceeds the threshold. At MVP scale with minimal platform fees, this is a "do before scaling" item, not a "do before launch" item. |
...
--   In a platform-as-custodian model, a settlements table tracks money
--   leaving the platform's bank account. In this model, money never
--   enters the platform — the gateway holds it and disburses it on our
--   instruction. Settle/refund lifecycle is tracked as columns on the
--   `payments` table itself. A separate settlements table would
...
## 5. Payment gateway abstraction (multi-provider support)

### 5.1 Why Flutterwave first, multi-gateway later

Flutterwave is the only major Nigerian gateway with a **native escrow API** (hold → settle → refund lifecycle). Paystack offers split payments but no hold period. InterSwitch offers neither natively.

For MVP, Flutterwave is the primary (and only) active gateway. The `PaymentGateway` trait still exists because:

- Future gateways may add escrow APIs
- Paystack can be used for the off-app path or simpler payment flows
- The abstraction costs ~2 hours of extra work today and avoids a rewrite later
- Buyer choice of payment method is a natural future enhancement

### 5.2 The `PaymentGateway` trait
...
/// Each gateway (Flutterwave, Paystack, InterSwitch) implements this trait.
/// The trait is object-safe and stored in AppState as `Arc<dyn PaymentGateway>`.
#[async_trait::async_trait]
pub trait PaymentGateway: Send + Sync {
    /// Human-readable name used in DB columns and logs (e.g., "flutterwave").
    ...
    /// For Flutterwave: POST /v2/gpx/transactions/escrow/settle
    /// For Paystack: no-op or split payment (no true escrow)
    async fn settle_escrow(
        &self,
        gateway_tx_id: &str,
    ) -> Result<GatewaySettleResponse, PaymentGatewayError>;

    /// Refund the buyer (full or partial).
    /// For Flutterwave: POST /v2/gpx/transactions/escrow/refund
    /// For Paystack: POST /refund
    ...
    /// Create a subaccount / transfer recipient for a seller's bank account.
    /// Returns the recipient/subaccount code.
...
#[derive(Debug, Clone)]
pub struct GatewaySettleResponse {
    pub success: bool,
    pub message: String,
}
...
### 5.3 Gateway-specific implementations
...
├── flutterwave.rs              # FlutterwavePaymentGateway impl (PRIMARY — MVP)
├── paystack.rs                 # PaystackPaymentGateway impl (future)
└── interswitch.rs              # InterSwitchPaymentGateway impl (future)
```

**Flutterwave implementation (`flutterwave.rs`) — PRIMARY for MVP:**

| Operation | API Endpoint |
|---|---|
| Initialize (with escrow) | `POST https://api.flutterwave.com/v3/payments` with `meta: [{metaname: "rave_escrow_tx", metavalue: 1}]` |
| Verify | `GET https://api.flutterwave.com/v3/transactions/{id}/verify` |
| Settle escrow | `POST https://api.flutterwave.com/v2/gpx/transactions/escrow/settle` |
| Refund escrow | `POST https://api.flutterwave.com/v2/gpx/transactions/escrow/refund` |
...
| Create subaccount | `POST https://api.flutterwave.com/v3/subaccounts` |

**Paystack implementation (`paystack.rs`) — future:**

| Operation | API Endpoint |
|---|---|
| Initialize (split payment) | `POST https://api.paystack.co/transaction/initialize` with `subaccount` param |
| Verify | `GET https://api.paystack.co/transaction/verify/{reference}` |
| Settle escrow | N/A — no native escrow; split happens at settlement |
...
| Resolve account | `GET https://api.paystack.co/bank/resolve` |
| Create subaccount | `POST https://api.paystack.co/subaccount` |

### 5.4 Gateway selection strategy

For MVP, Flutterwave is hardcoded as the active gateway:

```rust
// apps/api/src/core/config.rs — additions to Config struct

/// Payment gateway configuration. Currently only "flutterwave" is active.
pub default_payment_gateway: String,  // "flutterwave" (only option for MVP)

/// Flutterwave configuration (required for MVP)
pub flutterwave_secret_key: String,
pub flutterwave_public_key: String,
pub flutterwave_webhook_secret: String,

/// Paystack configuration (optional, for future use)
pub paystack_secret_key: Option<String>,
pub paystack_public_key: Option<String>,
pub paystack_webhook_secret: Option<String>,

/// InterSwitch configuration (optional, for future use)
...
/// Currently Flutterwave; Paystack/InterSwitch available via trait.
...
```
                    buyer chooses "Pay in-app" at reserve time
                          ...
                          pending_payment ──(gateway checkout abandoned/expired)──▶ failed
                                    │
                       (gateway webhook: charge.completed, signature verified)
                       [Flutterwave holds funds in escrow]
                                    │
                                    ▼
                          held_in_escrow ──(auto-release timer fires, no dispute)──▶ released
                                    │                     [gateway settle_escrow called]   │
                    ...
                    (buyer or seller opens a dispute)
                       ...
                       (admin resolves: release / refund / partial)
                        ...
                        ▼                       ▼
                    released                refunded
               [settle_escrow called]    [refund_escrow called]
```

**What happens at each transition:**

| Transition | Gateway action | Platform action |
|---|---|---|
| `pending_payment` → `held_in_escrow` | Webhook confirms payment; funds held by gateway in escrow | Lock row, set `held_at`, set `auto_release_at`, transition listing `reserved → sold` |
| `held_in_escrow` → `released` | Call `settle_escrow` API; gateway disburses to seller's bank | Lock row, set `released_at`, set `settle_status = 'settle_called'` |
| `held_in_escrow` → `refunded` | Call `refund_escrow` API; gateway refunds buyer | Lock row, set `refunded_at`, set `settle_status = 'refund_called'` |
| `settle_status` → `settled` | Gateway confirms settlement succeeded | Update `settle_at`, `settle_gateway_ref` |
| `settle_status` → `settle_failed` | Gateway reports settlement failed | Log error, increment `settle_attempts`, schedule retry |
...
**Description:** Create `payments`, `payment_disputes`, and `webhook_events` tables exactly as specified in §4.
**Acceptance Criteria:**
...
- `settle_status` CHECK constraint verified: invalid values rejected
- `sqlx migrate run` applies cleanly against a fresh DB

---

### CM-15.2 — `PaymentGateway` trait + `FlutterwavePaymentGateway` implementation

**Description:** Define the `PaymentGateway` trait (§5.2) and implement it for Flutterwave (§5.3). This is the primary gateway for MVP — the only one with a native escrow API.
**Acceptance Criteria:**

- `PaymentGateway` trait defined in `apps/api/src/core/clients/payment_gateway.rs` with all methods from §5.2
- `FlutterwavePaymentGateway` struct implements `PaymentGateway` in `apps/api/src/core/clients/flutterwave.rs`
- `initialize_transaction` calls `POST https://api.flutterwave.com/v3/payments` with `meta: [{metaname: "rave_escrow_tx", metavalue: 1}]` to enable escrow hold; returns the authorization URL
- `verify_transaction` calls `GET https://api.flutterwave.com/v3/transactions/{id}/verify`, maps Flutterwave's response to `GatewayVerifyResponse`
- `settle_escrow` calls `POST https://api.flutterwave.com/v2/gpx/transactions/escrow/settle` with transaction ID and secret key
- `refund_escrow` calls `POST https://api.flutterwave.com/v2/gpx/transactions/escrow/refund` with transaction ID, optional amount, and secret key
- `verify_webhook_signature` validates SHA-256 hash of (secret_key + raw_body) against `verif-hash` header
- `resolve_bank_account` calls `POST https://api.flutterwave.com/v3/accounts/resolve`
- `create_transfer_recipient` calls `POST https://api.flutterwave.com/v3/subaccounts`
- Amounts converted to Flutterwave's expected format at the boundary only — internal representation stays `Money` throughout
- Secret key loaded from `Config` (`FLUTTERWAVE_SECRET_KEY`), never logged
- Unit tests use `wiremock` (already a dev-dependency) to stub Flutterwave responses — no real network calls in tests

**Technical notes:**

- Flutterwave's NGN amounts are in kobo (their minor unit matches ours exactly for NGN).
- Lives at `apps/api/src/core/clients/flutterwave.rs`.
- `AppState` must be updated to include `payment_gateway: Arc<dyn PaymentGateway>` (§5.4).

---

### CM-15.3 — `POST /api/v1/payments/initiate`

**Description:** Buyer chooses the in-app payment path at reservation time; this endpoint starts a Flutterwave checkout for an already-reserved listing.
**Acceptance Criteria:**
...
- **Fee breakdown in response** (FCCPA compliance, §3.4):
  ```json
  {
...
**Endpoint:**
- `POST /api/v1/webhooks/flutterwave` — Flutterwave webhook receiver (MVP)

**Future endpoints:**
- `POST /api/v1/webhooks/paystack` — Paystack webhook receiver
- `POST /api/v1/webhooks/interswitch` — InterSwitch webhook receiver
...
├── webhook_flutterwave.rs    # Flutterwave-specific event parsing → calls webhook_common
├── webhook_paystack.rs       # Paystack-specific event parsing (future)
└── webhook_interswitch.rs    # InterSwitch-specific event parsing (future)
```

**Acceptance Criteria:**

- Verifies Flutterwave's `verif-hash` header (SHA-256 of secret_key + raw body) — requests with an invalid/missing signature are rejected with 401 **before** any DB write
...
- On `charge.completed`: locks the `payments` row (`FOR UPDATE`), verifies it's still `pending_payment`, transitions to `held_in_escrow`, sets `held_at = now()` and `auto_release_at = now() + escrow_window`, and — inside the **same transaction** — transitions the listing `reserved → sold` via the existing state-machine helper
...
- Endpoint is registered **outside** the standard auth middleware (Flutterwave can't send a JWT) but is not public in the trust sense — the signature check IS the authentication
...
**Description:** Buyer confirms they received the item, triggering the gateway to settle escrowed funds to the seller immediately instead of waiting for the auto-release window.
**Acceptance Criteria:**

- Requires `AuthUser`; caller must be the payment's `buyer_id` (403 otherwise)
- Payment must be `held_in_escrow` and `settle_status = 'not_initiated'` (409 otherwise — e.g. already released, or a dispute is open)
- Locks the row, transitions to `released`, sets `released_at`
- Calls `gateway.settle_escrow(gateway_tx_id)` — the gateway disburses `payout_amount` to the seller's bank account
- Sets `settle_status = 'settle_called'`
- If the settle call succeeds synchronously: `settle_status → 'settled'`, `settle_at = now()`
- If the settle call fails: `settle_status → 'settle_failed'`, `last_settle_error` logged, retry scheduled
...
**Description:** Periodic job that calls the gateway's settle endpoint automatically once `auto_release_at` passes, if no dispute was raised. Mirrors the shape of CM-4.8's stale-reservation job exactly.
**Acceptance Criteria:**

- Runs every few minutes, finds payments where `status = 'held_in_escrow' AND auto_release_at < now()` AND `settle_status = 'not_initiated'` AND no `open` dispute exists
...
- Calls `gateway.settle_escrow(gateway_tx_id)` and updates `settle_status` accordingly
...
- If the settle call fails, the job retries on the next tick (up to `MAX_SETTLE_ATTEMPTS`, suggested 5)

---

### CM-15.7 — Settle failure handling & retry

**Description:** Handles the case where `settle_escrow` fails (gateway down, invalid bank details, network error). The payment is in `released` state but the money hasn't actually reached the seller.
**Acceptance Criteria:**

- A background job scans for payments where `settle_status = 'settle_failed' AND settle_attempts < MAX_SETTLE_ATTEMPTS`
- Retries the settle call with exponential backoff (2min, 5min, 15min, 1hr, 4hr)
- On success: `settle_status → 'settled'`, `settle_at = now()`
- On final failure (max attempts reached): `settle_status` stays `settle_failed`, admin is alerted
- Admin can manually retry via `POST /api/v1/admin/payments/{id}/retry-settle`
- The payment status remains `released` throughout — the escrow was legitimately released from the buyer's perspective; the settle failure is a backend disbursement issue, not a buyer-facing problem
...
- `POST /api/v1/admin/payment-disputes/{id}/resolve` — body `{ resolution: "release" | "refund" | "partial", amount?: Money, note: String }`
  - `release` → calls `gateway.settle_escrow()`, payment `released`, dispute `resolved_release`, `settle_status` updated
  - `refund` → calls `gateway.refund_escrow()`, payment `refunded`, dispute `resolved_refund`, `settle_status → 'refund_called'`
  - `partial` → calls `gateway.refund_escrow(partial_amount)` and `gateway.settle_escrow(remainder)`, dispute `resolved_partial`
...
- **Refund timeline disclosure** (FCCPA compliance, §3.4): the response and UI must state the expected refund timeline (e.g., "Refund will be processed within 7 business days")

---

### CM-15.11 — Seller payout details collection

**Description:** One-time settings screen where a seller provides bank details, used to create a Flutterwave subaccount.
**Acceptance Criteria:**

- `POST /api/v1/users/me/payout-details` — bank code + account number, validated against Flutterwave's "resolve account number" endpoint before saving (confirms the account name matches, catches typos)
- On first submission: calls `gateway.create_transfer_recipient()` to create a Flutterwave subaccount; stores the `subaccount_code` (not raw bank details) on the user record
- Raw account numbers are never persisted after the subaccount is created, mirroring the "never store what you don't need" posture already used for refresh tokens (hash-only) elsewhere in this codebase
- A seller with no payout details on file cannot have their listing's payment auto-released — CM-15.5/15.6 checks for this and holds/flags instead of failing silently

---

### CM-15.12 — Transaction receipts & email notifications

**Description:** Send email receipts on key payment state changes, as required by FCCPA consumer protection rules (§3.4).
**Acceptance Criteria:**
...
- **On `released`:** send buyer a "payment released" email and seller a "you've been paid" email with payout details
...
- Full lifecycle test: initiate → webhook confirms → escrow held → confirm-receipt → settle_escrow called → settled, asserting `payout_amount` math is exact at every step (no float drift — this is the money-precision story from `core::money` paying off)
- Settle failure + retry test: settle_escrow fails → retry succeeds → settled
- Dispute flow test: dispute raised → auto-release blocked → admin resolves → settle or refund called
...
- A thin screen hosting a webview pointed at Flutterwave's `authorization_url` returned by `POST /payments/initiate`.
- On Flutterwave's redirect back to the app's callback URL, the app calls `GET /payments/{id}` (or polls briefly) to confirm status rather than trusting the redirect URL params alone — the webhook is still the source of truth, this screen just gives the user immediate feedback.
- **Must show fee breakdown** before the buyer confirms payment (FCCPA compliance, §3.4) — this can be rendered client-side from the `fee_breakdown` object returned by `POST /payments/initiate`.
- **Must show "Terms & Fees" link** pointing to a page explaining the platform's escrow process, fees, and dispute resolution (FCCPA compliance, §3.4).
- States: `PROCESSING…` → `PAYMENT CONFIRMED — FUNDS HELD IN ESCROW BY FLUTTERWAVE` (success) or `PAYMENT FAILED — TRY AGAIN OR ARRANGE IN PERSON` (failure, with a button back to the two-path choice).
...
```
PAYMENT STATUS: Funds held in escrow
Your payment is securely held by Flutterwave until you confirm delivery.
Auto-release in 2 days, or confirm now.

[ ✓ CONFIRM I RECEIVED THIS ITEM ]
[ ⚠ REPORT A PROBLEM ]

Transaction ID: uni_abc123
Payment method: Flutterwave (Card)
...
```
PAYMENT STATUS: Funds held in escrow by Flutterwave
Buyer has 3 days to confirm. Payout of ₦22,912.50 arrives automatically.

Settlement: Awaiting buyer confirmation
Transaction ID: uni_abc123
```
...
- A system message auto-posted into the chat thread on payment state changes ("Payment held in escrow by Flutterwave", "Buyer confirmed receipt — funds released to seller") so both parties have a shared record without needing to leave the conversation.
...
- **Settle failure queue**: list of payments where `settle_status = 'settle_failed'`, with retry and investigate actions.

### 8.8 Mockup/copy corrections

- The existing "Payment Screen" / "Checkout" mockups should be revised to reflect **gateway-held escrow language** — "Funds held securely by Flutterwave" not "Funds held by UniStash". Buyer protection and the confirm-receipt step are the actual value proposition of going in-app over cash, and the UI should say so explicitly.
...
> **so that** my funds are protected by Flutterwave's escrow until I actually receive the item.
...
- When I confirm, `POST /payments/initiate` runs, a `payments` row is created (`pending_payment`), and I'm shown the Flutterwave checkout webview.
- Given I complete payment on Flutterwave's page, when the webhook confirms `charge.completed`, then Flutterwave holds my funds in escrow. The platform transitions my payment to `held_in_escrow` and the listing to `sold`. I receive a payment confirmation email.
- Given I abandon or fail the Flutterwave checkout, when I return to the app, then my payment is `failed`, the listing is still `reserved`, and I'm offered "Try Again" or "Arrange In Person" instead.
...
> **so that** the seller gets paid by Flutterwave immediately.

- Given my payment is `held_in_escrow` and no dispute is open, when I tap "Confirm I Received This Item," then `POST /payments/{id}/confirm-receipt` triggers `gateway.settle_escrow()`, and Flutterwave disburses the payout to the seller's bank account.
- Given I take no action, when 72 hours pass with no dispute raised, then the auto-release job calls `gateway.settle_escrow()` on my behalf.
...
> **I want to** receive my payout from Flutterwave once escrow releases,
> **so that** I actually get paid for the sale.

- Given I haven't provided payout details yet, when my first in-app sale reaches `released`, then I'm prompted to add bank details (CM-15.11) before Flutterwave can settle.
- Given my payout details are on file (Flutterwave subaccount created), when a payment I'm the seller on transitions to `released`, then Flutterwave automatically disburses `payout_amount` to my bank account.
- I receive a "you've been paid" email with the payout amount and expected arrival time.
- If Flutterwave's settlement fails (rare), the admin is alerted and I'm notified — my money is not lost, it's in a retry queue.
...
- If settlement is processing or failed, I see the settle status too.
...
- Given I resolve a dispute as `refund`, then the buyer is refunded via Flutterwave's escrow refund API, the payment moves to `refunded`, and the action is written to `admin_audit_log`.
- Given I resolve as `release`, then Flutterwave settles the funds to the seller exactly as in Story 7.
...
2. **Escrow window length** — 72 hours is a starting guess. Too short and buyers can't inspect properly (especially with shipping delays, if this ever extends beyond same-campus pickup); too long and sellers feel like their money is stuck. Flutterwave's escrow hold period may also have limits — confirm.
3. **Flutterwave escrow hold limits** — Flutterwave's escrow API may have maximum hold periods or maximum amounts. Confirm these limits before relying on the 72-hour auto-release window and listing price ranges.
4. **Currency scope** — schema supports the existing `Currency` enum (NGN/USD/EUR/GBP), but Flutterwave's escrow API may only support NGN. For MVP, restrict in-app payments to NGN only.
...
6. **Transaction limits** — CBN tiered KYC limits (§3.3) may affect maximum listing prices for in-app payment. Should the platform enforce a maximum price for in-app-paid listings (e.g., ₦50,000 for Tier 1 users), or leave this to Flutterwave to enforce?
7. **Paystack as a fallback** — if Flutterwave has an outage, should the platform fall back to Paystack (no escrow, just split payment)? Or disable in-app payments entirely during an outage? This affects the gateway selection logic.
8. **DPIA completion** — a Data Protection Impact Assessment (§3.5) must be completed before go-live. This is a document deliverable, not code. Ensure someone owns this before the feature ships.
